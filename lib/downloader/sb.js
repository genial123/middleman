	...
	self.searcher.advanced(5, [0, 0, 0], [
		function(opts) {
			...
			if (opts.resp.data.quality.toLowerCase() != quality.sickname) {
				return opts.again('Episode-error: SickBeard snatched quality: '+opts.resp.data.quality+', but we want: '+quality.display);
			}

			self.logger.info('SickBeard has marked our episode-scan ('+quality.display+') as snatched, assuming everything went OK');
			opts.cb(null);
		}
	], callback);
};
	...
	self.searcher.advanced(5, [0, 0, 0], [
		function(opts) {
			...
			if ((typeof opts.success !== 'undefined') && opts.success) {
				self.logger.info('SickBeard has marked our season as snatched ('+quality.display+'), assuming everything went OK');
				return opts.cb(null);
			}

			opts.again('SickBeard scanned the season but still hasn\'t marked anything as snatched with quality: '+quality.sickname);
		}
	], callback);
};
