...
var q = require('./../utils/quality')(
	require('./../provider/btn/btn-quality').mapping
);
...
function validateMatch(show, release, test) {
	...
	if (!help.inArray(quality['key'], wanted)) return logger.info('Ignoring it, we don\'t want that quality');
...
function validateEpisode(show, release, quality, test) {
	sb.getEpisode(show['tvdbid'], release['season'], release['episode'], function(err, episode) {
		...
		if (proper) {
			...
			if (comp < 0) return logger.debug('Release quality is less than what we already have, ignoring');
			...
			if (comp < 1) return logger.debug('Release quality is less than or equal to what we already have, ignoring');
