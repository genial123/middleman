	...
	cp.doneMovies(function(err, movies) {
		...
		for (var i in movies) {
			...
			for (var y in movie.releases) {
				...
				if (typeof release.info === 'undefined') continue;
				if ((typeof release.info.id === 'undefined') || !release.info.id) continue;
				if (typeof release.info.provider === 'undefined') continue;
