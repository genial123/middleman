...
function describeBranch(branch, callback) {
	git(util.format('describe --tags %s', branch), function(err, out) {
		if (err) logger.debug(err.message);

		var vstring = out.trim();
		if (vstring) {
			var version = vstring.match(/^v(\d+)\.(\d+)\.(\d+)$/);
			...
			logger.error('Could not determine major.minor.patch using describe, version-string: '+vstring+', skipping version checking');
			return callback('Something went wrong, could not determine major.minor.patch for branch \''+branch+'\' using describe');
		}

		logger.error('Could not find branch version using describe, skipping version checking');
		callback('Something went wrong, could not find version for branch \''+branch+'\' using describe');
	});
}
...
function versionCompare(err, data, callback) {
	...
	var masterVersion = data[4];
	var originVersion = data[5];
...
exports.checkVersion = function(callback) {
	...
	async.series([
		...
		async.apply(describeBranch, 'master'),
		async.apply(describeBranch, 'origin/master')
	],
	function(err, data) {
