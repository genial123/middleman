...
function Searcher(namespace) {
	this.logger = require('./logger').get(namespace);
	this.queue = [];
	this.waiting = false;
	this.timer = null;
	this.waitfor = 5;
...
Searcher.prototype.__ready = function() {
	...
	var again = function(err) {
		if (err) self.logger.debug('Searcher returned error: '+err);
		...
		self.logger.debug('Scheduled new search '+(count+1)+'/'+max+' in '+wait+' seconds...');
		setTimeout(function() {
...
Searcher.prototype.__wait = function() {
	var self = this;
	self.logger.debug('Halting search-queue for '+self.waitfor+' seconds to avoid flooding');
...
Searcher.prototype.simple = function(delay, interval, attempts, fns, callback) {
	var self = this;
	self.logger.debug('Pushing new search (simple) into queue in '+delay+' second(s)...');
...
Searcher.prototype.advanced = function(delay, starts, fns, callback) {
	var self = this;
	self.logger.debug('Pushing new search (advanced) into queue in '+delay+' second(s)...');
	if (Object.prototype.toString.call(fns) != '[object Array]') fns = [fns];
...
exports.get = function(namespace) {
	if (typeof searchers[namespace] === 'undefined')
		searchers[namespace] = new Searcher(namespace);
	return searchers[namespace];
};
